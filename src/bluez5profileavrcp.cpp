	...
	BluetoothAvrcpPassThroughKeyStatus keyStatus)
{
	...
	{
		DEBUG("KeyKode: %d", keyCode);
		switch (keyCode)
		{
		case KEY_CODE_PAUSE:
		{
			DEBUG("AVRCP: Sending PAUSE");
			bluez_media_player1_call_pause_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_PLAY:
		{
			DEBUG("AVRCP: Sending PLAY");
			bluez_media_player1_call_play_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_STOP:
		{
			DEBUG("AVRCP: Sending STOP");
			bluez_media_player1_call_stop_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_REWIND:
		{
			DEBUG("AVRCP: Sending REWIND");
			bluez_media_player1_call_rewind_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_FAST_FORWARD:
		{
			DEBUG("AVRCP: Sending FAST_FORWARD");
			bluez_media_player1_call_fast_forward_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_NEXT:
		{
			DEBUG("AVRCP: Sending NEXT");
			bluez_media_player1_call_next_sync(mPlayerInterface, NULL, &error);
			break;
		}
		case KEY_CODE_PREVIOUS:
		{
			DEBUG("AVRCP: Sending PREVIOUS");
			bluez_media_player1_call_previous_sync(mPlayerInterface, NULL, &error);
			break;
		}
		default:
			DEBUG("Default, KeyKode: %d", keyCode);
		}
		if (error)
		{
			g_error_free(error);
